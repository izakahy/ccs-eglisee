  ...
  <div class="mx-auto">
    <div class="text-center">
      <div v-if="contents.bgIMG && contents.bgIMG.src" class="mb-8 relative overflow-hidden">
        <img :src="contents.bgIMG.src" alt="Gallery Header" class="w-full h-64 md:h-96 object-cover" loading="lazy" />
        <div v-if="contents.bgIMG.caption" class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 text-white p-3">
          <span class="px-5 py-3 rounded text-center text-4xl font-bold">{{ contents.bgIMG.caption }}</span>
        </div>
      </div>  
      
      <div v-if="contents.content1 && contents.content1.replace(/<[^>]*>/g, '').trim() !== ''" class="max-w-3xl flex flex-col mx-auto">
        <div v-html="contents.content1" class="editor-content prose lg:prose-xl text-center"></div>
        <div v-if="contents.content1.includes('<p')" class="flex justify-center mb-6">
            <div class="w-24 h-1 bg-emerald-500 rounded-full"></div>
          </div>
      </div>
    </div>
...
.editor-content :deep(h1) {
  @apply text-4xl font-bold text-indigo-900 mb-4;
}
.editor-content :deep(h2) {
  @apply text-3xl font-semibold text-indigo-800 mb-3;
}
