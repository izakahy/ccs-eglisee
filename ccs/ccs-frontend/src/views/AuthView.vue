...
<template>
    <h1 class="m-4 text-2xl lg:text-4xl text-center font-bold">Login to authenticate</h1>
    <!-- Added error/success message styling -->
    <div v-if="authStore.errors.auth" 
         class="mx-auto max-w-md mt-4 p-4 bg-red-100 text-red-700 rounded-lg">
        {{ authStore.errors.auth }}
    </div>
    
    <div v-if="authStore.message"
         class="mx-auto max-w-md mt-4 p-4 bg-green-100 text-green-700 rounded-lg">
        {{ authStore.message }}
    </div>
    <div class="min-h-screen bg-gray-100 flex flex-col justify-center sm:py-12">
        <div class="relative py-3 sm:max-w-xl sm:mx-auto">
            <div
            class="absolute inset-0 bg-gradient-to-r from-yellow-400 to-yellow-500 shadow-lg transform -skew-y-6 sm:skew-y-0 sm:-rotate-6 sm:rounded-3xl">
            </div>
